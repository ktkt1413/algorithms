# 백준 13975번 파일합치기3
# 문제: 소설가인 김대전은 소설을 여러 장(chapter)으로 나누어 쓰는데, 각 장은 각각 다른 파일에 저장하곤 한다.
...
for _ in range(int(input())):
    ...
    heapify(files)   #  리스트를 힙으로 변환함. 이는 최소 힙을 구성하게 된다. 최소힙은 가장 작은 값이 루트에 오도록 구성됨
    ans = 0
    for _ in range(n -1):
        a = heappop(files)  # 힙에서 가장 작은 파일을 꺼냄
        b = heappop(files)  # 힙에서 a 다음으로 작은 파일 꺼냄
        ans += a+b
        heappush(files, a+b)
    print(ans)